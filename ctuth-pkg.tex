...
\usepackage{hanging}
\usepackage{hhline}
\newfontfamily\techbf{Technika-Bold.otf}
\newfontfamily\tech{Technika-Book.otf}
\newfontfamily\techb{Technika-Regular.otf}
...
%	}
	\DeclareDocumentCommand\chapter{ s o m }{\relax
		...
		\hbox_to_wd:nn \linewidth {
			...
			% It can't be more than one line.
			\if@firstcolumn\hfil\fi
			\leavevmode\color{ctubluetext}\LARGE\strut\bfseries\techbf##3
			\if@firstcolumn\else\hfil\fi
		}\par
		\skip_vertical:n { 2pt }
		\everypar{\noindent}
...
\dim_const:Nn \g_ctuthesis_geometry_textwidth_dim { 15cm }
...
\ctuifswitch { layout-short } {
	\dim_const:Nn \g_ctuthesis_geometry_textheight_dim { 237mm }
	\cs_new:Nx \ctuthesis_geometry_vmarginratio: {4\string :4}
} {
	\dim_const:Nn \g_ctuthesis_geometry_textheight_dim { 237mm }
	\cs_new:Nx \ctuthesis_geometry_vmarginratio: {4\string :4}
}
...
\def\ttlh@ctuch#1#2#3#4#5#6#7#8{
	...
	\ctu@title@boxify{#5}{#3}{
		{#1\strut\ifttl@label{\techbf\Huge#2}\fi\par}
		\vspace{1pt}
		{\huge\color{ctubluetext}#4\strut#8\strut\par}
	}{}
}
...
\titleformat \paragraph [runin] {\normalfont\normalsize\techbf\sffamily} {\theparagraph} {1em} {\ctuthesis_title_adddot:n}
...
\hbox_set_to_wd:Nnn \g_ctuthesis_fancy_bluerule_l_box { 10.5pt } {
	{ \color{ctubluedot} \smash{\fontsize{28.88}{0}\fontfamily{lmss}\techbf.} }
	\hfil
}
...
\DeclareCaptionFormat{myformat}{%
  % float name bf sf
  \begingroup\sffamily\techbf#1#2:~\endgroup
...
\cs_new:Nn \ctuthesis_list_prepare_labelitem:nnnn {
	...
	\hbox_gset:Nn \g_ctuthesis_tempa_box {
		\scalebox{#3}{\fontfamily{lmss}\techbf\color{#4}.}
	}

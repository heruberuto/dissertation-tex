...
\label{chap:contribution}
	...
	extit{We collected novel data for fact‑checking in our application context, emulated or scraped unavailable datasets (publishing or preparing them for release), established strong baseline models, and are working to position claim generation as a summarization‑adjacent NLP task.}

\section{Datasets}
With the automated fact‑checking scheme established in Chapter~\ref{chap:sota}, any ML solution begins with choosing or collecting appropriate training data.
...
This design was relatively cheap (translating the entire 2017 Wikipedia would be costly and wasteful), yielding a CC‑licensed dataset of 127K claims, labels, and evidence. Since both 2017 EnWiki and our 2020 CsWiki corpora contained only the first paragraph (abstract) per article, we hoped document‑level alignment would hold: both languages summarize basic facts about the same entity.
...
While noisy, \FCZ has been used to train IR components~\cite{rypar,gazo,lrev} still in use today and is openly available\footnote{\url{https://huggingface.co/datasets/ctu-aic/csfever}} under a CC license.
...
The application has two layers: a Yii‑based PHP interface for annotation and a Python Flask service hosting our IR models based on TF‑IDF~\cite{drqa} and mBERT (Section~\ref{sec:bert}), trained (among other data) on \FCZ (Section~\ref{sec:fcz}).
...
I used this multiplicity to assess data quality and task ambiguity, and to propose cleaning methods for the final cleaned \CTK dataset.
...
Krippendorff's alpha was 56.42\% and Fleiss' $\kappa$ 63\%, an adequate result reflecting the complexity of news‑based verification within a fixed knowledge scope.
...
The \CTK dataset then underwent thorough human‑in‑the‑loop cleaning to reach 100\% agreement on the retained items, removing obvious noise and revealing sources of annotation error.
...
These datasets are now standard benchmarks within the AIC NLP group~\cite{semin,mlynar} and are beginning to appear in external research~\cite{stefanik}.
...
\section{Models}
\label{sec:models}
The most significant released models address two tasks: Natural Language Inference and Claim Generation (as Abstractive Summarization).
...
\begin{itemize}
    ...
    \item The original English FEVER NLI task\footnote{\url{https://huggingface.co/ctu-aic/xlm-roberta-large-squad2-enfever_nli}}~\cite{fever,nie2019combining}, achieving 75.9\% macro‑F1 and surpassing the prior shared‑task winner~\cite{nie2019combining} (69.5 macro‑F1 with NSMNs)
    \end{enumerate}
\end{itemize}
...
Another interactive application (Figure~\ref{fig:factsearch}), developed by Jan Drchal~\cite{mlynar}, demonstrates our best‑performing end‑to‑end fact‑checking models, integrating XLM‑RoBERTa trained on \FCZNLI.
